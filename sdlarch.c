...
static void create_window(int width, int height) {
    ...
    g_win = SDL_CreateWindow("SdlArch", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, 0);
...
static void video_refresh(const void *data, unsigned width, unsigned height, unsigned pitch) {
    ...
    SDL_Texture *tex = SDL_CreateTextureFromSurface(g_renderer, screen);
    SDL_FreeSurface(screen);
    SDL_RenderCopy(g_renderer, tex, NULL, NULL);
